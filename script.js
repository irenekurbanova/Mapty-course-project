...
class App {
  ...
  _newWorkout(e) {
    ...
    // Add new object tot workout array
  ...
  _deleteWorkout(e) {
    const workoutEl = e.target.closest('.workout');
    const workoutDeleteBtn = e.target.closest('.btn--delete');

    if (!workoutEl) return;
    if (!workoutDeleteBtn) return;

    const workoutToDelete = this.#workouts.find(
      i => i.id == workoutEl.dataset.id
    );

    const workoutToDeleteIndex = this.#workouts.indexOf(workoutToDelete);

    this._deleteById(workoutToDeleteIndex);
    workoutEl.remove();

    this._removeMarker(workoutToDeleteIndex);
